...
function FamilyPensionGraph({ type, reloadGraph }: any) {
  ...
  const { summaryData, summaryLoading, error } = useSummaryQuery(selectedDate);
  ...
  useEffect(() => {
    try {
      ...
      setUniqueCategories(["Regular", "Family"]);

      // Find the matching month data

      if (selectedGraphTab === "Overview") {
        // For Pie Chart: Show Regular & Family separately
        ...
        const formattedOverview = [
          {
            ...
            value:
              selectedTab === "count"
                ? monthData.regular.count
                : monthData.regular.amount,
          },
          {
            name: "Family",
            value:
              selectedTab === "count"
                ? monthData.family.count
                : monthData.family.amount,
          },
        ];
        setPieData(formattedOverview);
      } else {
        // For Line Chart: Show trends for both Regular & Family
        const trendData = data.map((item) => ({
          month: item.month,
          Regular:
            selectedTab === "count" ? item.regular.count : item.regular.amount,
          Family:
            selectedTab === "count" ? item.family.count : item.family.amount,
        }));

        console.log("Formatted Line Chart Data:", trendData);
        setPieData(trendData);
      }
    } catch (error) {
...
export default FamilyPensionGraph;
