...
// Add type definition for dataset
...
const PerformanceIndex: React.FC<PerformanceIndexProps> = ({
  ...
  //variance view
  const [selecteddivisionsName, setSelecteddivisionsName] =
    useState<string[]>(divisionsName);
  ...
  // Handle apply button click for trend
  const handleApplyTrend = async () => {
    ...
    try {
      ...
      if (diffMonths > 4) {
        setTrendError("Date range cannot be more than 4 months");
        return;
      }

      if (fromDate > toDate) {
        setTrendError("From date cannot be after To date");
        return;
      }

      // Format dates as MM/YYYY
      const fromDateStr = `${fromMonthIndex
        .toString()
        .padStart(2, "0")}/${fromYear}`;
  ...
  // Effect to automatically fetch trend data when selections change
  useEffect(() => {
    if (view === "trend") {
      handleApplyTrend();
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (data && data.data) {
          // Get the data for the selected type and month
          const typeData = data.data[type][monthYear] || {};
          console.log("typeData", typeData);
          // Transform the data to match the expected format
          const transformedData = Object.entries(typeData).map(
            ([division, values]: [string, any]) => ({
              Division: division,
              Description: selectedMetric,
              MonthYear: monthYear,
              Cumulative_Target: values.targetYTDThisMonth || 0,
              Cumulative_Actual_CFY: values.actualYTDThisMonth || 0,
              Cumulative_Actual_Last_Year:
                values.actualYTDThisMonthLastYear || 0,
              Target: values.targetThisMonth || 0,
              Actual_CFY: values.actualThisMonth || 0,
              Actual_Last_Year: values.actualThisMonthLastYear || 0,
            })
          );
    ...
    datasets: [
      {
        label: "Jaipur",
        data: [
          performanceData.find((item) => item.Division === "Jaipur")?.Target ||
            0,
          performanceData.find((item) => item.Division === "Jaipur")
            ?.Actual_CFY || 0,
          performanceData.find((item) => item.Division === "Jaipur")
            ?.Actual_Last_Year || 0,
        ],
      ...
      },
      {
        label: "Jodhpur",
        data: [
          performanceData.find((item) => item.Division === "Jodhpur")?.Target ||
            0,
          performanceData.find((item) => item.Division === "Jodhpur")
            ?.Actual_CFY || 0,
          performanceData.find((item) => item.Division === "Jodhpur")
            ?.Actual_Last_Year || 0,
        ],
      ...
      },
      {
        label: "Ajmer",
        data: [
          performanceData.find((item) => item.Division === "Ajmer")?.Target ||
            0,
          performanceData.find((item) => item.Division === "Ajmer")
            ?.Actual_CFY || 0,
          performanceData.find((item) => item.Division === "Ajmer")
            ?.Actual_Last_Year || 0,
        ],
      ...
      },
      {
        label: "Bikaner",
        data: [
          performanceData.find((item) => item.Division === "Bikaner")?.Target ||
            0,
          performanceData.find((item) => item.Division === "Bikaner")
            ?.Actual_CFY || 0,
          performanceData.find((item) => item.Division === "Bikaner")
            ?.Actual_Last_Year || 0,
        ],
  ...
  const processTrendChartData = (
            ...
            if (item.Division === division && item.Description === category) {
              ...
              if (monthIndex !== -1) {
                ...
                if (metric === "Target" && item.Target !== undefined) {
                  value = item.Target;
                } else if (metric === "Actual" && item.Actual_CFY !== undefined) {
                  value = item.Actual_CFY;
                } else if (metric === "LFY" && item.Actual_Last_Year !== undefined) {
                  value = item.Actual_Last_Year;
                } else if (metric === "CFY YTD Target" && item.Cumulative_Target !== undefined) {
                  value = item.Cumulative_Target;
                } else if (metric === "CFY YTD Actual" && item.Cumulative_Actual_CFY !== undefined) {
                  value = item.Cumulative_Actual_CFY;
                } else if (metric === "LFY YTD Actual" && item.Cumulative_Actual_Last_Year !== undefined) {
                  value = item.Cumulative_Actual_Last_Year;
                } else if (metric === "Variance From Target" && item.Cumulative_Target !== undefined && item.Cumulative_Actual_CFY !== undefined) {
                  value = item.Cumulative_Actual_CFY - item.Cumulative_Target;
                } else if (metric === "Variance From LFY" && item.Cumulative_Actual_Last_Year !== undefined && item.Cumulative_Actual_CFY !== undefined) {
                  value = item.Cumulative_Actual_CFY - item.Cumulative_Actual_Last_Year;
                }
    ...
    // Add NWR datasets only if showNWR is true
    if (showNWR) {
  ...
  const chartData = processTrendChartData(
    trendData,
    selecteddivisionsName,
    selectedCategories,
    selectedMetrics,
  ...
  const fetchTrendData = async (
    ...
    try {
      ...
      // Convert category to the correct type for the API
      const type =
        selectedCategories[0] === "EARNINGS"
          ? "sumearning"
          : selectedCategories[0] === "EXPENDITURE"
          ? "sumexpenditure"
          : "pei";
      ...
      if (data && data.data) {
        ...
        const transformedData = Object.entries(typeData)
          .filter(([date]) => {
          ...
          .flatMap(([date, divisionData]: [string, any]) => {
            return Object.entries(divisionData).map(
              ([division, values]: [string, any]) => ({
                Division: division,
      ...
      {view === "data" ? (
        ...
        >
          ...
          <Box sx={{ mb: 2, display: "flex", gap: 2, alignItems: "center" }}>
            ...
            <Typography color="white" variant="body2" sx={{ ml: 2 }}>
              To:
            </Typography>
            <Select
              value={toMonth}
