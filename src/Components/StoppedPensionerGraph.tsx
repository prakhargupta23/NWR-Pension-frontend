...
function StoppedPensionerGraph({ type, reloadGraph }: any) {
  ...
  const { data, loading } = useFetchDataForPensionWithRetry({
    service: sqlService,
    method: "fetchStoppedPensionerData",
  ...
  useEffect(() => {
    try {
      ...
      setUniqueCategories(["Stopped Pensioners"]);
...
export default StoppedPensionerGraph;
