...
function AgeGraph({ type, reloadGraph }: any) {
  ...
  const [data, setData] = useState<any[]>([]); // Stores full data
    ...
    const fetchDataWithRetry = async (retries = 2) => {
      ...
      try {
        ...
        if (retries > 0) {
          ...
          console.error("fetchAgeData failed after multiple attempts:", err);
        }
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    try {
      ...
      console.log("Filtering for:", selectedMonthYear);
      console.log("Raw data:", data);

      // Extract unique categories dynamically (keys except "month")
      ...
      const key = selectedTab === "amount" ? "amount" : "count"; // Corrected key selection
      ...
      if (selectedGraphTab === "Overview") {
        ...
        const trendData = data.map((item) => {
          const formattedItem: any = { month: item.month };

          uniqueCategories.forEach((category) => {
            formattedItem[category] = item[category]?.[key] ?? 0; // Handle missing values safely
          });
