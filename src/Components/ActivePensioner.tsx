...
import DownloadButton from "../modules/Button";
...
function ActivePensionerGraph({ type, reloadGraph }: any) {
  ...
  const { data, loading } = useFetchDataForPensionWithRetry({
    service: sqlService,
    method: "fetchActivePensionerData",
  ...
  useEffect(() => {
    try {
      ...
      setUniqueCategories(["Active Pensioners"]);
        ...
        >
          ...
          >
            ...
            >
              ...
              {selectedGraphTab === "Overview" ? (
                <DownloadButton selectedDate={selectedDate} />
              ) : null}
            </Grid>
          </Grid>
          {loading ? (
...
export default ActivePensionerGraph;
